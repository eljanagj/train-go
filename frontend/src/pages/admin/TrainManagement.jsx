...
const TrainManagement = ({ theme, toggleTheme }) => {
  ...
  const validateForm = () => {
    ...
    } else if (newTrain.productionYear < 1900 || newTrain.productionYear > new Date().getFullYear()) {
  ...
  const updateTrain = async (trainID) => {
    ...
    try {
      const { trainID: id, createdAt, updatedAt, schedules, totalCapacity, availableSeats, ...updateData } = editingTrain;
      ...
      if (originalTrain && originalTrain.status !== editingTrain.status) {
        console.log(`Updating status from ${originalTrain.status} to ${editingTrain.status}`);
          ...
          <div className="management-form-body">
            <div className="form-grid">
              <div className="form-group">
                ...
                />
                {formErrors.trainName && <span className="error-message">{formErrors.trainName}</span>}
              </div>
              <div className="form-group">
              ...
              <div className="form-group">
                ...
                />
                {formErrors.productionYear && <span className="error-message">{formErrors.productionYear}</span>}
              </div>
              <div className="form-group">
              ...
              <div className="form-actions">
                <button
                  ...
                  onClick={createTrain}
                  disabled={!newTrain.trainName.trim() || !newTrain.model.trim() || !newTrain.manufacturer.trim() || !newTrain.productionYear || !newTrain.status}
                >
                  <FaPlus /> Add Train
                </button>
                  ...
                  {editingTrain && editingTrain.trainID === train.trainID ? (
                    <>
                      ...
                      <td className="action-buttons">
                        <button className="save-button" onClick={() => updateTrain(train.trainID)}>
                          <FaEdit /> Save
                        </button>
                    ...
                    <>
                      ...
                      <td>{train.status}</td>
                      <td className="action-buttons">
                        <button className="edit-button" onClick={() => startEditing(train)}>
                          <FaEdit /> Edit
                        </button>
                        <button className="delete-button" onClick={() => handleDeleteClick(train)}>
                          <FaTrash /> Delete
                        </button>
                        <button
